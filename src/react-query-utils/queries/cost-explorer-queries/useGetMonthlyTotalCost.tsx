...
const useGetMonthlyTotalCost = () => {
    ...
    const linkAWSMonthlyConsole = `https://console.aws.amazon.com/cost-reports/home?region=${currentAwsRegion}#/dashboard?reportName=${currentStack}+Cost+Explorer&timePeriod.start=${startDate}&timePeriod.end=${endDate}&granularity=MONTHLY&groupBy=SERVICE`
    ...
    if (monthlyCostQuery.isSuccess && monthlyCostQuery.data?.ResultsByTime) {
        ...
        formattedData = Object.keys(totalCostByService).map(serviceName => ({
            Service: serviceName,
    ...
    return {
        ...monthlyCostQuery, linkAWSMonthlyConsole, formattedData
    }
}
